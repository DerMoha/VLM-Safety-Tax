...
def _get_generation_adapter_spec(
    ...
    return AdapterSpec(
        ...
        temperature=0.0,
        random=None,
        output_mapping_pattern=output_mapping_pattern,
    )
...
def _get_short_answer_generation_adapter_spec(instructions: Optional[str] = None) -> AdapterSpec:
    return _get_generation_adapter_spec(
        ...
        max_tokens=4096, # To allow output for R1 with think tags
    )
...
def _get_multiple_choice_joint_adapter_spec(
    ...
    num_outputs: int = 1,
    *,
    max_tokens: int = 4096, # To allow output for R1 with think tags
    stop_sequences: Optional[List[str]] = None,
    # Robust extraktion eines einzigen Buchstabens (A–E), falls Model mehr ausgibt
    output_mapping_pattern: Optional[str] = r"([A-E])",
) -> AdapterSpec:
    return AdapterSpec(
        ...
        instructions=(
            ...
            "Output ONLY a single capital letter from [A-E]."
        ),
        ...
        max_tokens=max_tokens,
        stop_sequences=(stop_sequences if stop_sequences is not None else ["</think>"]),
        temperature=0.0,
        random=None,
        output_mapping_pattern=output_mapping_pattern,
    )
...
def get_mscoco_categorization_spec() -> RunSpec:
    ...
    adapter_spec: AdapterSpec = _get_multiple_choice_joint_adapter_spec(
        input_noun=None, output_noun="Answer", max_train_instances=0, max_tokens=8
    )
...
def get_math_vista_spec(grade: str, question_type: str) -> RunSpec:
    ...
    if question_type == "free_form":
        ...
        adapter_spec = _get_multiple_choice_joint_adapter_spec(
            input_noun=None, output_noun="Answer", max_train_instances=0, max_tokens=4096 # To allow output for R1 with think tags
        )
    else:
